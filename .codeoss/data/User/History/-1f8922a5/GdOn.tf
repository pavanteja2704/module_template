resource "google_monitoring_alert_policy" "policy" {
    ...
    dynamic "condition_threshold" {
      ...
      condition_threshold {
        ...
        dynamic "denominator_aggregations" {
          ...
          for_each                       = var.denominator_aggregations[*]
          content {
            alignment_period         = lookup(denominator_aggregations.value, "alignment_period", null)
            per_series_aligner       = lookup(denominator_aggregations.value, "per_series_aligner", null)
            cross_series_reducer     = lookup(denominator_aggregations.value, "cross_series_reducer", null)
      ...
      dynamic "condition_matched_log" {
        ...
        for_each                       = var.condition_matched_log[*]
        content {
       ...
       dynamic "condition_prometheus_query_language" {
        # for_each                    = lookup(conditions.value, "condition_prometheus_query_language", [])
        ...
        #  trigger                    =  lookup(condition_prometheus_query_language.value, "trigger", [])
        #  evaluation_missing_data    = lookup(condition_prometheus_query_language.value, "evaluation_missing_data", null )
  ...
  dynamic "alert_strategy" {
     for_each                       = lookup(conditions.value,"alert_strategy", [])
      content {
       auto_close                   = lookup(lookup(each.value, "alert_strategy", {}), "auto_close", null)
      }
  
      notification_rate_limit {
       period                       =    lookup(lookup(each.value, "notification_rate_limit", {}), "period", null)
     }

     notification_channel_strategy {
       notification_channel_names   =   lookup(lookup(each.value, "notification_channel_strategy", {}), "notification_channel_names", null)
       renotify_interval            =  lookup(lookup(each.value, "notification_channel_strategy", {}), "renotify_interval", null)
     }
  }

  dynamic "documentation" {
    for_each                        = lookup(conditions.value,"documentation", [])
      ...
      content {
       ...
       subject                      =lookup(lookup(each.value, "documentation", {}), "subject", "Generated with terraform")
     }
 }
