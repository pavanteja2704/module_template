  ...
  dynamic "conditions" {
    ...
    dynamic "condition_threshold" {
      ...
      dynamic "condition_matched_log" {
         for_each                    = lookup(conditions.value, "condition_matched_log", null)
        content {
       ...
       dynamic "notification_rate_limit" {
        for_each                    = lookup(conditions.value, "notification_rate_limit", [])
        content {
     ...
     dynamic "notification_channel_strategy" {
      for_each                      = lookup(conditions.value, "notification_channel_strategy", [])
      content {
